...
const STOPWORDS = new Set(
  [
    'من', 'في', 'على', 'مع', 'عن', 'الى', 'او', 'ثم', 'حتى', 'لكن', 'يا', 'ما',
    'مش', 'بس', 'اللي', 'ده', 'دي', 'دول', 'هو', 'هي', 'هم', 'انت', 'انتي',
    'انا', 'احنا', 'انتو', 'لا', 'ولا', 'ان', 'الله', 'ربنا', 'حد', 'ايه',
    'ازاي', 'ليه', 'فين', 'امتى', 'كده', 'اي', 'كل', 'زي', 'عشان', 'علشان',
    'برضه', 'بردو', 'قوي', 'خالص', 'يعني', 'طبعا', 'اه', 'ايوه'
  ].map((w) =>
    w
      .replace(/[أإآٱ]/g, 'ا')
      .replace(/ى/g, 'ي')
...
const GLOSS_STOPWORDS = new Set([
  'to', 'a', 'an', 'the', 'of', 'and', 'or', 'be', 'is', 'are', 'was', 'were',
  'it', 'in', 'on', 'at', 'for', 'with', 'from', 'that', 'this', 'his', 'her',
  'someone', 'something', 'one', 'not', 'no', 'yes', 'do', 'doe', 'made', 'make'
]);
 ...
 *
 * Undiacritized Arabic cannot distinguish السُّكّر ("diabetes") from السكر
 * ("sugar"), or ورد ("veins") from ورد ("replied"), so skeleton matching alone
 * produces examples that illustrate a different word. Requiring the gloss to
 * show up in the translation is a cheap, surprisingly strong check on that.
 */
function exampleMatchesSense(english: string, tokens: string[]): boolean {
...
function toSlug(franco: string, arabic: string, taken: Set<string>): string {
  let base = franco
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-|-$/g, '');
  if (!base) base = encodeURIComponent(normalise(arabic)).replace(/%/g, '').toLowerCase().slice(0, 24);
...
async function main() {
  ...
  // Index sentences by every token form they contain.
      ...
      JSON.stringify(
        {
          ...
          audioUrl: tableRow?.audio_url ?? null,
