...
<section class="px-3 mt-6 sm:px-8 max-w-5xl mx-auto">
  ...
  {#if fushaGeneratedStories.length === 0}
    <div class="text-center py-12">
      ...
      <h2 class="text-2xl text-text-300 font-bold mb-4">No Fusha Stories Yet</h2>
      <p class="text-text-200 mb-6">Create your first Modern Standard Arabic story to get started!</p>
    </div>
  {:else}
    <h2 class="text-2xl text-text-300 font-bold mb-4">AI-Generated Fusha Stories</h2>
    <ul class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 auto-rows-fr">
      {#each fushaGeneratedStories as story}
      <li class="flex">
        <a href={`/fusha/generated-stories/${story.id}`} class="flex w-full">
