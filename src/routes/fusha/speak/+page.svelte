<script lang="ts">
  ...
  let index = $state((() => {
    if (typeof window !== 'undefined') {
      ...
      const urlIndex = parseInt(params.get('speak_sentence_fusha') ?? '0') || 0;
  ...
  let sentences = $state((() => {
    if (typeof window !== 'undefined') {
      const savedSentences = localStorage.getItem('speak_sentence_fusha');
      if (savedSentences) {
  ...
  async function processVocabularyFile(file: File): Promise<string> {
    ...
    localStorage.setItem('speak_sentence_fusha', JSON.stringify(updatedSentences));
		isLoading = false;
	}
	...
	function next() {
		...
		index = index + 1;
    updateUrl('speak_sentence_fusha', (index+1).toString());
	}
  ...
  function resetSentences() {
    sentences = [];
    index = 0;
    localStorage.removeItem('speak_sentence_fusha');
    updateUrl('speak_sentence_fusha', '0');
  }
...
{:else if sentences.length > 0 && index < sentences.length && !hasReachedLimit && data.session}
	...
	<SpeakSentence {sentence} {resetSentences} dialect="fusha" />
{/if}
