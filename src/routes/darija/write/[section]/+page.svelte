<script>
  ...
  const isPaywalled = darijaSections.find(section => section.path === data.section)?.isPaywalled ?? false;
</script>
...
{:else}
	...
	<div class="px-3 mt-6 sm:px-8 max-w-5xl mx-auto">
		{#if word.english}
			<WriteWordBlock {word} {mode} switchMode={handleSwitchMode} dialect="darija" />
		{/if}
	</div>
{/if}
