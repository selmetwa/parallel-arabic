<script lang="ts">
  ...
  let index = $state((() => {
    if (typeof window !== 'undefined') {
      ...
      const urlIndex = parseInt(params.get('sentence_darija') ?? '0') || 0;
  ...
  let sentences = $state((() => {
    if (typeof window !== 'undefined') {
      const savedSentences = localStorage.getItem('sentences_darija');
      if (savedSentences) {
  ...
  async function processVocabularyFile(file: File): Promise<string> {
	...
	function next() {
		...
		index = index + 1;
    updateUrl('sentence_darija', (index+1).toString());
	}
  ...
  function resetSentences() {
    sentences = [];
    index = 0;
    localStorage.removeItem('sentences_darija');
    updateUrl('sentence_darija', '0');
  }
			...
			<form class="flex flex-col gap-4 p-6" onsubmit={generateSentences}>
				<div class="text-left mb-4">
					<h1 class="text-xl sm:text-2xl text-text-300 font-bold mb-2 tracking-tight">
						Practice your darija skills with generated sentences.
					</h1>
...
{:else if sentences.length > 0 && index < sentences.length && !hasReachedLimit && data.session}
	...
	{#if mode === 'write'}
		<section class="px-3 sm:px-8 max-w-6xl mx-auto py-4">
			<SentenceBlock {sentence} {resetSentences} dialect="darija" />
		</section>
	{/if}
