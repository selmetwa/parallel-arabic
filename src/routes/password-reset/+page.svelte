...
<section class="flex flex-col gap-4 px-4 pt-4 sm:mt-12 sm:flex-row sm:px-20">
	<div class="flex-2">
        ...
        loading = true;
				return async ({ result }) => {
					if (result.type === 'success') {
						message = { type: 'success', text: 'Password reset was link sent to your email' };
					} else {
		...
		>
			<h1 class="text-xl font-semibold text-text-300">Reset password</h1>
			{#if message}
	...
	<div class="flex-3 border border-tile-600 bg-tile-400">
		<ul class="flex flex-col gap-3 p-8">
			...
			<li>
				...
				<p class="text-md text-text-200">
					Practice writing over <b>20,000</b> Arabic words with a virtual arabic keyboard custom built
			...
			<li>
				...
				<p class="text-md text-text-200">
					Practice over <b>20,000</b> words with fun multiple choice quizzes
				</p>
			</li>
			<li>
