<script lang="ts">
	...
	const practicePhrases = $derived<PracticeWord[]>(
...
<div class="mx-auto max-w-5xl px-4 py-8">
	...
	<p class="mb-6 text-text-200">
		How to say the everyday things in {dialectName}, with the Arabic script, the tashkeel, a
		plain-ASCII transliteration, and the forms you need for a man, a woman, or a group.
	</p>
	...
	{#if data.phrases.length === 0}
		<div class="rounded-xl border border-tile-500 bg-tile-300 py-12 text-center">
			...
			<p class="text-sm text-text-200">
				Run <code class="rounded bg-tile-500 px-1">npm run generate:phrases</code> to build the phrasebook.
			</p>
		</div>
	...
	<div class="mt-10">
		<VocabPractice
			words={practicePhrases}
			dialect={data.dialect as Dialect}
			topicLabel="these phrases"
