<script lang="ts">
	...
	import { levantineGeneratedStoryInStore } from '$lib/store/store';
	...
	async function handleSubmit(event: any) {
		...
		const res = await fetch('/api/create-story', {
			...
			body: JSON.stringify({
				option,
				description,
        title: title + '_levantine', // Add dialect suffix
        dialect: 'levantine' // Specify dialect for story generation
			})
		});
		...
		isLoading = false;
    await goto('/levantine/generated-stories/' + chatgptres.storyId);
	}
		...
		{#if isLoading}
			...
			>
				...
				<p class="text-2xl text-text-300">
					Generating your Levantine story, hang tight. <br />
					this usually takes a few seconds.
				</p>
			</div>
		{:else}
			<h1 class="text-2xl font-semibold text-text-300">Create Levantine Arabic Story</h1>
      <p>
        <i>Story is AI generated in Levantine Arabic and may contain mistakes</i>
      </p>
...
<div class="w-fit mt-2">
  <Button onClick={openModal} type="button">Create your own Levantine story</Button>
</div> 