<script lang="ts">
  ...
  let index = $state((() => {
    if (typeof window !== 'undefined') {
      ...
      const urlIndex = parseInt(params.get('speak_sentence_levantine') ?? '0') || 0;
  ...
  let sentences = $state((() => {
    if (typeof window !== 'undefined') {
      const savedSentences = localStorage.getItem('speak_sentence_levantine');
      if (savedSentences) {
  ...
  async function processVocabularyFile(file: File): Promise<string> {
    ...
    localStorage.setItem('speak_sentence_levantine', JSON.stringify(updatedSentences));
		isLoading = false;
	}
	...
	function next() {
		...
		index = index + 1;
    updateUrl('speak_sentence_levantine', (index+1).toString());
	}
  ...
  function resetSentences() {
    sentences = [];
    index = 0;
    localStorage.removeItem('speak_sentence_levantine');
    updateUrl('speak_sentence_levantine', '0');
  }
...
{:else if sentences.length > 0 && index < sentences.length && !hasReachedLimit && data.session}
	...
	<SpeakSentence {sentence} {resetSentences} dialect="levantine" />
{/if}
