...
<WordModal {activeWordObj} {isModalOpen} {closeModal} dialect="iraqi"></WordModal>
  ...
  <header class="border-b border-tile-600 px-4 pb-8 text-center sm:px-8">
    ...
    <div class="flex flex-col items-start justify-between sm:flex-row sm:items-center">
      <div class="flex w-fit gap-2">
          ...
          <AudioButton text={sentences[index].arabic.text} dialect="iraqi">
            Play Audio
          </AudioButton>
          {/if}
          {#if data.story?.[0]?.id}
          <StoryAudioButton storyId={data.story[0].id} dialect="iraqi" />
          {/if}
      </div>
