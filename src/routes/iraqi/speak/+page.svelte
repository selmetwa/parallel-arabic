<script lang="ts">
  ...
  let index = $state((() => {
    if (typeof window !== 'undefined') {
      ...
      const urlIndex = parseInt(params.get('speak_sentence_iraqi') ?? '0') || 0;
  ...
  let sentences = $state((() => {
    if (typeof window !== 'undefined') {
      const savedSentences = localStorage.getItem('speak_sentence_iraqi');
      if (savedSentences) {
  ...
  // Remove mode state since it's not needed for speaking
	...
	function next() {
		...
		index = index + 1;
    updateUrl('speak_sentence_iraqi', (index+1).toString());
	}
  ...
  function resetSentences() {
    sentences = [];
    index = 0;
    localStorage.removeItem('speak_sentence_iraqi');
    updateUrl('speak_sentence_iraqi', '0');
  }
...
{:else if sentences.length > 0 && index < sentences.length && !hasReachedLimit && data.session}
	...
	<SpeakSentence {sentence} {resetSentences} dialect="iraqi" />
{/if}
