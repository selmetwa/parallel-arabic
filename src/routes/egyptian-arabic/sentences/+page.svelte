<script lang="ts">
  ...
  let index = $state((() => {
    if (typeof window !== 'undefined') {
      ...
      const urlIndex = parseInt(params.get('sentence_egyptian') ?? '0') || 0;
  ...
  async function processVocabularyFile(file: File): Promise<string> {
    ...
    localStorage.setItem('sentences_egyptian', JSON.stringify(updatedSentences));
		isLoading = false;
	}
	...
	function next() {
		...
		index = index + 1;
    updateUrl('sentence_egyptian', (index+1).toString());
	}
