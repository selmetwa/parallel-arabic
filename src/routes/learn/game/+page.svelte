<script lang="ts">
	...
	import { GAME_MODE_INFO, GAME_FAQS } from '$lib/constants/game-content';
	...
	function getModeDescription(mode: string): string {
		switch (mode) {
			...
			case 'listening':
				return 'Listen to audio and select the correct word or meaning';
			case 'speaking':
...
<section class="mx-auto max-w-4xl px-3 py-6 sm:px-8">
	<h1 class="mb-3 text-3xl font-bold tracking-tight text-text-300 sm:text-4xl">Arabic Games</h1>

	<p class="mb-6 text-lg leading-relaxed text-text-200">
		Free Arabic vocabulary games in Egyptian Arabic, Levantine, Moroccan Darija and Modern Standard
		Arabic. Pick a dialect and a topic, choose whether you want to read, listen or speak, and play.
		Nothing to install, and your progress is saved so you can pick a game back up where you left it.
	</p>

	<!-- In-Progress Games -->
	{#if inProgressGames.length > 0}
		<div class="mb-6 rounded-xl border border-tile-500 bg-tile-300 p-4">
			...
			<div class="space-y-3">
				{#each inProgressGames as game}
					<div
						class="flex flex-col gap-4 rounded-lg border border-tile-500 bg-tile-400 p-3 sm:flex-row sm:items-center"
					>
						<div class="flex min-w-0 flex-1 items-center gap-3">
							<span class="text-2xl">{getDialectEmoji(game.dialect)}</span>
							<div class="min-w-0 flex-1">
								<p class="truncate font-semibold text-text-300">
						...
						<div class="flex items-center gap-4 sm:gap-6">
							...
							<div class="text-center">
								<p class="text-lg font-bold text-text-300">
									{game.current_index}/{game.total_questions}
								</p>
							...
							<div class="hidden text-center sm:block">
								<p class="text-sm font-medium text-text-200">
							...
							<div class="flex gap-2">
								<button
									...
									class="rounded-lg bg-sky-500 px-4 py-2 font-semibold text-white transition-all hover:bg-sky-600 active:scale-95"
								>
									Continue
								</button>
								<button
									...
									class="rounded-lg p-2 text-text-100 transition-all hover:bg-tile-500 hover:text-rose-400 disabled:opacity-50"
									title="Delete game"
								>
									{#if deletingGameId === game.id}
										<div
											class="h-5 w-5 animate-spin rounded-full border-2 border-text-100 border-t-transparent"
										></div>
									{:else}
	...
	<div class="space-y-4">
		...
		<div class="rounded-xl border border-tile-500 bg-tile-400 p-4">
			<h2 class="mb-3 text-lg font-bold text-text-300">1. Word Source</h2>

			{#if !useCustomWords}
				<div>
					<label for="category" class="mb-2 block text-sm font-semibold text-text-300">
						Category
					</label>
					<select
						id="category"
						bind:value={selectedCategory}
						class="w-full rounded-lg border border-tile-500 bg-tile-300 px-3 py-2.5 text-text-300 focus:border-blue-500 focus:outline-none"
					>
						{#each availableCategories as category}
							<option value={category.path}>{category.name} ({category.count} words)</option>
						{/each}
					</select>
					<p class="mt-1 text-sm text-text-100">You'll practice all words in this category</p>
				</div>
			{:else}
				<div class="space-y-4">
					<!-- Content Type: Words or Sentences -->
					<div>
						<label class="mb-2 block text-sm font-semibold text-text-300"> Content Type </label>
						<div class="flex gap-3">
							<button
								onclick={() => (contentType = 'words')}
								class="flex-1 rounded-lg px-4 py-3 font-semibold transition-all {contentType ===
								'words'
									? 'bg-blue-500 text-white shadow-lg'
									: 'border border-tile-500 bg-tile-300 text-text-200 md:hover:bg-tile-500'}"
							>
								Words
							</button>
							<button
						...
						{#if contentType === 'sentences'}
							<div class="mt-2 rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-3">
								<p class="text-sm text-text-300">
									Sentences help with context-based learning - you'll understand how words are used
									in real conversations!
								</p>
							</div>
						{:else}
							<p class="mt-1 text-sm text-text-100">Practice individual vocabulary words</p>
						{/if}
					</div>
					...
					{#if contentType === 'sentences'}
							...
							<div class="rounded-lg border border-tile-500 bg-tile-300 p-4">
								<div class="flex items-center justify-between gap-4">
									...
									<label class="relative inline-flex cursor-pointer items-center">
										<input type="checkbox" bind:checked={useReviewWords} class="peer sr-only" />
										<div
											class="peer h-7 w-12 rounded-full bg-tile-500 after:absolute after:left-[2px] after:top-[2px] after:h-6 after:w-6 after:rounded-full after:bg-white after:transition-all after:content-[''] peer-checked:bg-emerald-600 peer-checked:after:translate-x-full peer-checked:after:border-white peer-focus:outline-none"
										></div>
									</label>
								</div>
									...
									<div class="mt-4 space-y-3">
										<div class="flex gap-2">
											<button
												...
												class="flex-1 rounded-lg px-3 py-2 text-sm font-semibold transition-all {reviewWordsSource ===
												'all'
													? 'bg-blue-500 text-white'
													: 'border border-tile-600 bg-tile-400 text-text-200 md:hover:bg-tile-500'}"
											>
												All Saved Words
											</button>
											<button
										...
										{:else if reviewWordsError}
											<p class="text-sm text-rose-400">{reviewWordsError}</p>
						...
						{:else}
							<div class="rounded-lg border border-tile-500 bg-tile-300 p-3">
								<p class="text-sm text-text-200">
									<a href="/login" class="text-blue-400 hover:text-blue-300">Log in</a> to use your saved
						...
						<div>
							...
							<div class="grid grid-cols-2 gap-2 sm:grid-cols-4">
								{#each learningTopics as topic}
									<button
										...
										class="flex items-center justify-center gap-1 rounded-lg px-3 py-2 text-sm font-medium transition-all {selectedLearningTopics.includes(
											topic.id
										)
											? 'bg-sky-600 text-white shadow'
											: 'border border-tile-600 bg-tile-300 text-text-200 md:hover:bg-tile-500'}"
									>
							...
							<p class="mt-1 text-xs text-text-100">Select specific grammar topics to practice</p>
						</div>
					{/if}

					<div>
						<label for="customTopic" class="mb-2 block text-sm font-semibold text-text-300">
						...
						<input
							...
							class="w-full rounded-lg border border-tile-500 bg-tile-300 px-3 py-2.5 text-text-300 focus:border-blue-500 focus:outline-none"
						/>
						<p class="mt-1 text-sm text-text-100">Leave empty for random {contentType}</p>
					</div>

					<div>
						<label for="difficulty" class="mb-2 block text-sm font-semibold text-text-300">
							Difficulty Level
						</label>
						<select
							id="difficulty"
					...
					<div>
						<label class="mb-2 block text-sm font-semibold text-text-300">
						...
						<div class="flex flex-wrap gap-3">
							{#each [5, 10, 15, 20] as count}
								<button
									...
									class="rounded-lg px-6 py-3 font-semibold transition-all {questionCount === count
										? 'bg-blue-500 text-white shadow-lg'
										: 'border border-tile-500 bg-tile-300 text-text-200 md:hover:bg-tile-500'}"
								>
									{count}
		...
		<!-- Game Mode Selection -->
		<div class="rounded-xl border border-tile-500 bg-tile-400 p-4">
			<h2 class="mb-3 text-lg font-bold text-text-300">2. Game Mode</h2>
			<div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
				{#each ['multiple-choice', 'listening', 'speaking'] as mode}
					<button
						onclick={() => (selectedMode = mode as typeof selectedMode)}
						class="flex flex-col items-center gap-2 rounded-xl p-4 transition-all {selectedMode ===
						mode
							? 'bg-blue-500 text-white shadow-md'
							: 'border border-tile-500 bg-tile-300 text-text-200 md:hover:bg-tile-500'}"
					>
						<span class="text-2xl">{getModeIcon(mode)}</span>
						<span class="text-lg font-bold capitalize">{mode.replace('-', ' ')}</span>
						<span class="text-center text-sm opacity-80">{getModeDescription(mode)}</span>
					</button>
				{/each}
			</div>
		</div>
		...
		<button
			onclick={startGame}
			class="w-full rounded-xl bg-emerald-600 px-8 py-3 text-lg font-bold text-white shadow-sm transition-all active:scale-[0.98] md:hover:bg-emerald-700 md:hover:shadow-md"
		>
			Start Game
		</button>
	</div>

	<section class="mt-14 border-t border-tile-600 pt-10">
		<h2 class="mb-6 text-2xl font-bold text-text-300 sm:text-3xl">The game modes</h2>
		<div class="grid gap-4 sm:grid-cols-3">
			{#each GAME_MODE_INFO as mode (mode.title)}
				<div class="rounded-xl border border-tile-500 bg-tile-300 p-5">
					...
					<h3 class="mb-2 text-lg font-bold text-text-300">{mode.title}</h3>
					<p class="text-sm leading-relaxed text-text-200">{mode.body}</p>
				</div>
			{/each}
		</div>
	</section>
	...
	<section class="mt-12">
		<h2 class="mb-6 text-2xl font-bold text-text-300 sm:text-3xl">Common questions</h2>
		<div class="space-y-4">
			{#each GAME_FAQS as faq (faq.question)}
				<div class="rounded-xl border border-tile-500 bg-tile-300 p-5">
	...
	<nav class="mt-10 border-t border-tile-600 pt-6 text-sm text-text-200">
		<a class="underline hover:text-text-300" href="/vocabulary">Browse the vocabulary</a>
		<span class="mx-2">·</span>
		<a class="underline hover:text-text-300" href="/alphabet">Learn the alphabet</a>
		<span class="mx-2">·</span>
		<a class="underline hover:text-text-300" href="/stories">Read a story</a>
	</nav>
</section>
