<script lang="ts">
  ...
  let index = $state((() => {
    if (typeof window !== 'undefined') {
      ...
      const urlIndex = parseInt(params.get('speak_sentence_khaleeji') ?? '0') || 0;
  ...
  let sentences = $state((() => {
    if (typeof window !== 'undefined') {
      const savedSentences = localStorage.getItem('speak_sentence_khaleeji');
      if (savedSentences) {
  ...
  async function processVocabularyFile(file: File): Promise<string> {
    ...
    localStorage.setItem('speak_sentence_khaleeji', JSON.stringify(updatedSentences));
		isLoading = false;
	}
	...
	function next() {
		...
		index = index + 1;
    updateUrl('speak_sentence_khaleeji', (index+1).toString());
	}
  ...
  function resetSentences() {
    sentences = [];
    index = 0;
    localStorage.removeItem('speak_sentence_khaleeji');
    updateUrl('speak_sentence_khaleeji', '0');
  }
...
{:else if sentences.length > 0 && index < sentences.length && !hasReachedLimit && data.session}
	...
	<SpeakSentence {sentence} {resetSentences} dialect="khaleeji" />
{/if}
