<script lang="ts">
  ...
  // Filter stories for khaleeji dialect and remove the dialect suffix from title
  let khaleejiGeneratedStories = $derived.by(() => {
    const output = []
    ...
    for (const story of data.user_generated_stories) {
      ...
      // Remove the _khaleeji suffix from the title for display
      const displayTitle = story.title.replace('_khaleeji', '');
...
<section class="px-3 mt-6 sm:px-8 max-w-5xl mx-auto">
  <div class="text-left mb-6">
    <h1 class="text-3xl sm:text-4xl text-text-300 font-bold mb-1 tracking-tight">Khaleeji Stories</h1>
    ...
    <CreateStoryModal dialect="khaleeji" data={data}></CreateStoryModal>
  </div>
  ...
  {:else}
    ...
    <ul class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 auto-rows-fr">
      {#each khaleejiGeneratedStories as story}
      <li class="flex">
        <a href={`/khaleeji/generated-stories/${story.id}`} class="flex w-full">
