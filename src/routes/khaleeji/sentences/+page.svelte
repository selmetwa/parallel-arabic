<script lang="ts">
  ...
  import SentenceQuiz from '$lib/components/dialect-shared/sentences/SentenceQuiz.svelte';
	import { khaleejiSentencesInStore, currentDialect } from '$lib/store/store';
  ...
  let index = $state((() => {
    if (typeof window !== 'undefined') {
      ...
      const urlIndex = parseInt(params.get('sentence_khaleeji') ?? '0') || 0;
  ...
  let sentences = $state((() => {
    if (typeof window !== 'undefined') {
      const savedSentences = localStorage.getItem('sentences_khaleeji');
      if (savedSentences) {
        const parsed = JSON.parse(savedSentences);
        return filterValidSentences(parsed);
      }
    }
    const storeSentences = Array.isArray($khaleejiSentencesInStore) ? $khaleejiSentencesInStore : [];
  ...
  async function processVocabularyFile(file: File): Promise<string> {
    ...
    if (vocabularyInputMode === 'file' && vocabularyFile) {
      try {
		...
		khaleejiSentencesInStore.set(updatedSentences as any);
    localStorage.setItem('sentences_khaleeji', JSON.stringify(updatedSentences));
		isLoading = false;
	}
	...
	function next() {
		...
		index = index + 1;
    updateUrl('sentence_khaleeji', (index+1).toString());
	}
  ...
  function resetSentences() {
    sentences = [];
    index = 0;
    localStorage.removeItem('sentences_khaleeji');
    updateUrl('sentence_khaleeji', '0');
  }
			...
			<form class="flex flex-col gap-4 p-6" onsubmit={generateSentences}>
				<div class="text-left mb-4">
					<h1 class="text-xl sm:text-2xl text-text-300 font-bold mb-2 tracking-tight">
						Practice your khaleeji skills with generated sentences.
					</h1>
...
{:else if sentences.length > 0 && index < sentences.length && !hasReachedLimit && data.session}
	...
	{#if mode === 'write'}
		<section class="px-3 sm:px-8 max-w-6xl mx-auto py-4">
			<SentenceBlock {sentence} {resetSentences} dialect="khaleeji" />
		</section>
	{/if}
