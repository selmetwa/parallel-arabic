<script lang="ts">
  ...
  let sentences = $state((() => {
    if (typeof window !== 'undefined') {
      const savedSentences = localStorage.getItem('speak_sentence');
      if (savedSentences) {
  ...
  // Remove mode state since it's not needed for speaking
	...
	async function generateSentance(option: string, copy: any) {
		...
		sentencesInStore.set(updatedSentences as any);
    localStorage.setItem('speak_sentence', JSON.stringify(updatedSentences));
		isLoading = false;
	}
  ...
  function resetSentences() {
    sentences = [];
    index = 0;
    localStorage.removeItem('speak_sentence');
